...
### Date and Water Type Line Plot

```{r}
# historical_filter2 <- historical_production %>%
#   filter(org_id %in% c(376, 2158, 2629, 2631, 372, 2132, 2140, 2683, 2130),
#          !is.na(quantity_acre_feet)) %>% 
#   group_by(org_id, start_date, water_produced_or_delivered, water_type) %>% 
#   summarise(quantity = sum(quantity_acre_feet)) %>% 
#   pivot_wider(names_from = water_produced_or_delivered,
#               values_from = quantity) 
# 
# historical_filter2$surplus <- historical_filter2$`water produced` - historical_filter2$`water delivered`
# 
# historical_filter2 <- historical_filter %>% 
#   mutate(county = case_when(
#     org_id %in% c(376, 2158, 2629, 2631) ~ "Ventura",
#     org_id %in% c(372, 2132, 2140, 2683, 2130) ~ "Sacremento",
#     TRUE ~ "other"
...
# Filter to only produced 
historical_produce <- historical_filter2 %>% 
  filter(water_produced_or_delivered == "water produced")

# Line plot 
ggplot(historical_produce, aes(x = start_date, y = quantity, color = water_type)) + geom_line() +
  facet_wrap(~county) +
  theme(
    legend.position = "none"
  )
...
# Density plot, giving a weird looking graph 
# ggplot(historical_produce, aes(x = start_date, group = quantity, fill = water_type)) + geom_density() +
#   facet_wrap(~county)
```
